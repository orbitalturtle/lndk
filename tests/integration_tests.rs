...
async fn test_lndk_send_invoice_request() {
    ...
    let offer = ldk1
        .create_offer(&path_pubkeys, Network::Regtest, 20_000, Quantity::One)
        .await
    ...
    let client = lnd.client.unwrap().clone();
    ...
    select! {
        ...
        res = request_invoice(client, lnd_pubkey, offer, pubkey_2, blinded_path) => {
            assert!(res.is_ok());
