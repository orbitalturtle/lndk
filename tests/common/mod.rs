...
pub async fn setup_test_infrastructure(
    ...
    TempDir,
    ElectrsD,
    Arc<Node<FilesystemStore>>,
    TempDir,
) {
    let (bitcoind, bitcoind_dir) = setup_bitcoind().await;
    let lnd_exe_dir = download_lnd().await;

    let lnd_node = LndNode::new(bitcoind.params.clone(), lnd_exe_dir, test_name);
...
async fn setup_ldk(esplora_url: String) -> (Arc<Node<FilesystemStore>>, TempDir) {
    let builder = LdkBuilder::new();
