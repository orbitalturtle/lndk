...
impl LndNode {
    ...
    pub async fn get_info(&mut self) -> tonic_lnd::lnrpc::GetInfoResponse {
        let get_info_req = tonic_lnd::lnrpc::GetInfoRequest {};
        ...
        let resp = if let Some(ref mut client) = &mut self.client {
            ...
            while retry_num < 3 {
                thread::sleep(Duration::from_secs(2));
                match client.lightning().get_info(get_info_req.clone()).await {
                    Err(_) => {
                        println!("retrying get_info call");
                        retry_num += 1;
                        if retry_num == 3 {
                            panic!("get_info call failed after 3 retries");
                        }
                        continue;
                    }
    ...
    pub async fn connect_to_peer(
        ...
        let resp = if let Some(ref mut client) = &mut self.client {
            ...
            while retry_num < 3 {
                thread::sleep(Duration::from_secs(2));
                match client.lightning().connect_peer(connect_req.clone()).await {
                    Err(_) => {
                        println!("retrying connect_peer call");
                        retry_num += 1;
                        if retry_num == 3 {
                            panic!("connect_peer call failed after 3 retries");
                        }
                        continue;
                    }
                    Ok(resp) => return resp.into_inner(),
