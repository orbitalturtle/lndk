use crate::common::ldk_node::convert::{
...
use crate::common::ldk_node::hex_utils;
use base64;
...
impl WalletSource for BitcoindClient {
    fn list_confirmed_utxos(&self) -> Result<Vec<Utxo>, ()> {
        let utxos = tokio::task::block_in_place(move || {
            self.handle
                .block_on(async move { self.list_unspent().await })
                .0
        });
        Ok(utxos
            .into_iter()
    ...
    fn get_change_script(&self) -> Result<Script, ()> {
        tokio::task::block_in_place(move || {
            Ok(self
                .handle
                .block_on(async move { self.get_new_address().await.script_pubkey() }))
        })
    }
    ...
    fn sign_tx(&self, tx: Transaction) -> Result<Transaction, ()> {
        ...
        let signed_tx = tokio::task::block_in_place(move || {
            self.handle
                .block_on(async move { self.sign_raw_transaction_with_wallet(tx_hex).await })
        });
