...
use crate::common::ldk_node::{
    hex_utils, ChannelManager, FilesystemPersister, PENDING_SPENDABLE_OUTPUT_DIR,
};
...
pub(crate) async fn periodic_sweep(
    ...
    let pending_spendables_dir = format!("{}/{}", PENDING_SPENDABLE_OUTPUT_DIR, ldk_data_dir);
    ...
    loop {
        ...
        if let Ok(dir_iter) = fs::read_dir(&pending_spendables_dir) {
            ...
            if !outputs.is_empty() {
                ...
                persister
                    .persist(
                        &format!("spendable_outputs/{}", key),
                        &WithoutLength(&outputs),
                    )
                    .unwrap();
