...
pub enum OfferError<Secp256k1Error> {
    ...
    /// OnionMessengerFailure indicates a problem from the onion messenger.
    OnionMessengerFailure(OnionMessengerError),
}
...
impl Display for OfferError<Secp256k1Error> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            OfferError::OnionMessengerFailure(e) => write!(f, "Error from onion messenger: {e:?}"),
        }
    }
}
...
#[derive(Debug)]
pub enum OnionMessengerError {
    PeerConnectFailure,
    SendError(SendError),
}

impl Display for OnionMessengerError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            OnionMessengerError::PeerConnectFailure => write!(f, "Error connecting to peer."),
            OnionMessengerError::SendError(e) => write!(f, "Error sending onion message: {e:?}"),
        }
    }
}

impl Error for OnionMessengerError {}
...
pub async fn request_invoice(
    client: impl MessageSigner + std::marker::Send + 'static + Clone,
    pubkey: PublicKey,
    offer: Offer,
    ...
    let node_client = signer_clone.signer().await;
...
pub async fn send_invoice_request(
    ...
    onion_messenger
        ...
        .map_err(|_| OfferError::OnionMessengerFailure(OnionMessengerError::PeerConnectFailure))?;
    ...
    onion_messenger
        ...
        .map_err(|e| OfferError::OnionMessengerFailure(OnionMessengerError::SendError(e)))
}
